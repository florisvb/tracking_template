...
def draw_optic_flow_field(gray_image, points):
    ...
    for i, point in enumerate(points):
        x = int(point[0]) # note: opencv drawing requires ints
        y = int(point[1])
...
NOISE = scipy.stats.norm(0,0.05) # gaussian noise
def get_randomized_sinusoidal_point(xy_frequencies=[[4,5]], noise=True): # defaults to one point
    points = []
    for xy_frequency in xy_frequencies:
        if noise:
            points.append([np.sin(time.time()*xy_frequency[0]*np.pi) + NOISE.rvs(), np.sin(time.time()*xy_frequency[1]*np.pi) + NOISE.rvs(), 0])
        else: # no noise
            points.append([np.sin(time.time()*xy_frequency[0]*np.pi), np.sin(time.time()*xy_frequency[1]*np.pi), 0])
    return points
...
class Image_Processor:
    def __init__(self):
        ...
        self.tracked_points_pub = rospy.Publisher("/tracking_template/tracked_points", PointCloud)
    ...
    def image_callback(self,image):
        try: # if there is an image
            ...
            # tracked_points = get_random_tracked_points() # for example - this would return a random number of random points
            tracked_points = get_randomized_sinusoidal_point()
            #########################################################################
